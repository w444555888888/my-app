...
const DynamicFormList: React.FC<DynamicFormListProps> = ({ name, label, fields, addButtonText = '新增' }) => {
  return (
    <Form.List name={name}>
        ...
        <div>
          {fieldItems.map(({ key, name: itemName, ...restField }) => (
            <Space key={key} align="baseline" style={{ display: 'flex', marginBottom: 8 }}>
              ...
              <Button danger onClick={() => remove(itemName)}>刪除</Button>
            </Space>
          ))}
          <Form.Item>
            <Button type="dashed" onClick={() => add()} block>{addButtonText}</Button>
          </Form.Item>
        </div>
      )}
    </Form.List>
