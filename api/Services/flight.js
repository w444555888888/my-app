...
export const updateFlightService = async (id, updateData) => {
    ...
    if (updateData.route?.arrivalCity) {
    ...
    if (updateData.schedules && updateData.schedules.length > 0) {
        ...
        const updatedSchedules = updateData.schedules.map(schedule => {
            ...
            for (const cabin of flight.cabinClasses) {
                availableSeats[cabin.category] = cabin.totalSeats;
                prices[cabin.category] = cabin.basePrice;
            }
...
export const getAllFlightsService = async (queryParams) => {
    ...
    if (isSearchMode) {
        ...
        query["schedules.departureDate"] = { $gte: new Date(startDate), $lte: new Date(endDate) };
    }
    ...
    const flights = await Flight.find(query)
        .select({
            flightNumber: 1,
            route: 1,
            cabinClasses: { category: 1, basePrice: 1, totalSeats: 1 },
            schedules: { departureDate: 1, arrivalDate: 1, availableSeats: 1, prices: 1 }
        })
        .lean();
